...
def realanimate():
    ...
    flat_list1 = [item for sublist in C1 for item in sublist]
    flat_list2 = [item for sublist in C2 for item in sublist]
    flat_list3 = [item for sublist in C3 for item in sublist]
    count=0
    mx=0
    steps=300
    rc1=[]
    for i in range(0, len(flat_list1)):
        rc1=flat_list1[0::steps]
    rc1.append(flat_list1[-1])
    print(len(rc1))
    if(len(rc1)>mx):
        mx=len(rc1)
    count=0
    rc2=[]
    for i in range(0, len(flat_list2)):
        rc2=flat_list2[0::steps]
    rc2.append(flat_list2[-1])
    print(len(rc2))
    if(len(rc2)>mx):
        mx=len(rc2)
    count=0
    rc3=[]
    for i in range(0, len(flat_list3)):
        rc3=flat_list3[0::steps]
    rc3.append(flat_list3[-1])
    print(len(rc3))
    if(len(rc3)>mx):
        mx=len(rc3)    
    for i in range(len(rc1), mx):
        rc1.append(rc1[-1])
    
    for i in range(len(rc2), mx):
        rc2.append(rc2[-1])
        
    for i in range(len(rc3), mx):
        rc3.append(rc3[-1])

    df1 = pd.DataFrame(rc1, columns=['theta1','phi1','alpha1'])
    df2 = pd.DataFrame(rc2, columns=['theta2','phi2','alpha2'])
    df3 = pd.DataFrame(rc3, columns=['theta3','phi3','alpha3'])
    df=pd.concat([df1,df2,df3], axis=1)
    
    #new_df.columns =['X_axis', 'Y_axis', 'Z_axis', 'Radius']
    json_df_concat = df.to_json(orient='records')
    #json_df2 = df2.to_dict(orient='records')
